...
import balabit_feature_extraction.general_statistics as gs
import balabit_feature_extraction.settings as st
import balabit_feature_extraction.time_sample_interpolation as ts


# computes features from an action
# input:
# returns a tuple representing a feature vector
def computeActionFeatures(x, y, t, actionCode, action_sequence, n_from, n_to):
    n = len(x)
    if st.GLOBAL_DEBUG:
    ...
    result = (actionCode, trajectory, time, direction,
              straightness, n, sumOfAngles,
              mean_curv, sd_curv, max_curv, min_curv,
              mean_omega, sd_omega, max_omega, min_omega,
              largest_deviation,
              distEndToEndLine, numCriticalPoints,
...
def getAction(x, y, t, actionCode, action_file, n_from, n_to):
    return computeActionFeatures(x, y, t, actionCode, action_file, n_from, n_to)
...
# one PC action
def processPointClick(x, y, t, action_sequence, n_from, n_to):
    return getAction(x, y, t, st.PC, action_sequence, n_from, n_to)


# one MM action
def processMouseMove(x, y, t, action_sequence, n_from, n_to):
    return getAction(x, y, t, st.MM, action_sequence, n_from, n_to)

# one DD action
def processDragAction(x, y, t, action_sequence, n_from, n_to):
    return getAction(x, y, t, st.DD, action_sequence, n_from, n_to)


# can be a compound action: {MM}*PC
def processPointClickActions(data, action_sequence, n_from, n_to):
    if st.GLOBAL_DEBUG:
    ...
    for item in data:
        ...
        if actState == 'Pressed':
            if len(t) > st.GLOBAL_MIN_ACTION_LENGTH:
                ...
                action_sequence.append(processPointClick(x, y, t, action_sequence, start, n_to))
            ...
            if actTime - prevTime > st.GLOBAL_DELTA_TIME:
                ...
                if len(t) > st.GLOBAL_MIN_ACTION_LENGTH:
                    ...
                    action_sequence.append(processMouseMove(x, y, t, action_sequence, start, stop))
                # uj AcTION kezdodik
                x = []
                y = []
                t = []
...
def processMouseMoveActions(data, action_sequence, n_from, n_to):
    if st.GLOBAL_DEBUG:
    ...
    if len(t) > st.GLOBAL_MIN_ACTION_LENGTH:
        ...
        action_sequence.append(processMouseMove(x, y, t, action_sequence, start, n_to))
    return action_sequence

# {MM}*DD
def processDragActions(data, action_sequence, n_from, n_to):
    if st.GLOBAL_DEBUG:
    ...
    for item in data:
        ...
        if actButton == 'Left' and actState == 'Released':
            ...
            action_sequence.append(processDragAction(x, y, t, action_sequence, start, n_to))
